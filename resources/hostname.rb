...
action :set do
  ...
  if node['platform_family'] != 'windows'
    ...
    if ::File.exist?('/usr/sbin/scutil')
      ...
      if ::File.exist?('/usr/bin/hostnamectl') && !docker_guest?
      ...
      elsif ::File.exist?('/etc/sysconfig/network')
      ...
      elsif ::File.exist?('/etc/conf.d/hostname')
        # Gentoo
    ...
    elsif ::File.exist?('/usr/sbin/svccfg')
      ...
      execute "svccfg -s system/identity:node setprop config/nodename=\'#{new_resource.hostname}\'" do
        ...
        # not_if { shell_out!('svccfg -s system/identity:node listprop config/nodename').stdout.chomp =~ /config\/nodename\s+astring\s+#{new_resource.hostname}/ } TODO: replaced to satisfy cookstyle, but maybe the pattern should include the leading slash?
        not_if { shell_out!('svccfg -s system/identity:node listprop config/nodename').stdout.chomp =~ %r{config/nodename\s+astring\s+#{new_resource.hostname}/} } # satisfies cookstyle
      end
